...
import { ArrowRight, MessageSquare, FileText, Download, CreditCard } from "lucide-react";
...
export default async function PortalDashboard() {
  ...
  type ActivityItem = { id: string; label: string; sub: string; href: string; time: Date; icon: "update" | "file" | "invoice" };
              ...
              {activity.map((item) => (
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                    item.icon === "file" ? "bg-blue-500/10" : "bg-primary/10"
                  }`}>
                    {item.icon === "file" ? (
